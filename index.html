...
function renderMapScreen() {
  ...
  screen.innerHTML = `
    <section class="card map-screen-card fade-in accent-border-${identity.accent}">
  ...
  bindActions({
    "back-to-game": () => renderResume(identity)
  });
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", async () => {
    try {
      const registration = await navigator.serviceWorker.register("./service-worker.js?v=045", {
        updateViaCache: "none"
      });
